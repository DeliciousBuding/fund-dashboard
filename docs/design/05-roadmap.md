...
- W4：`POST/PUT/DELETE /api/dca/plans`（upsert/disable）、`POST /api/dca/run`（dry_run 支持）、`POST /api/securities`（标的入册）、`POST /api/portfolio/adjust-position`
- W6：`check_alerts`、`generate_report` 翻 REST；`/api/agent/tools*` 只读面接受 SessionAuth（供设置页 Agent 面）
