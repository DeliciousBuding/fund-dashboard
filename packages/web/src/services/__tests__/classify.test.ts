...
describe('getMarketLabel', () => {
  it('returns market.label.sh for sh codes', () => {
    expect(getMarketLabel('600519')).toBe('market.label.sh');
  });

  it('returns market.label.sz for sz codes', () => {
    expect(getMarketLabel('000001')).toBe('market.label.sz');
  });

  it('returns market.label.hk for hk codes', () => {
    expect(getMarketLabel('00700')).toBe('market.label.hk');
  });

  it('returns market.label.us for us codes', () => {
    expect(getMarketLabel('AAPL')).toBe('market.label.us');
  });
...
describe('STOCK_MARKETS', () => {
  it('has sh, sz, hk, us entries with labelKey', () => {
    ...
    expect(STOCK_MARKETS.sh.labelKey).toBe('market.label.sh');
    expect(STOCK_MARKETS.sz.labelKey).toBe('market.label.sz');
    expect(STOCK_MARKETS.hk.labelKey).toBe('market.label.hk');
    expect(STOCK_MARKETS.us.labelKey).toBe('market.label.us');
  });
});

// ── CATS and CAT_ORDER ──────────────────────────────────────────
describe('CATS and CAT_ORDER', () => {
  it('CATS has expected categories with nameKey and Chinese keyword funds', () => {
    ...
    expect(CATS.nasdaq.nameKey).toBe('category.nasdaq');
...
// ── STOCK_CATS ──────────────────────────────────────────────────
describe('STOCK_CATS', () => {
  it('has stock-a, stock-hk, stock-us with nameKey', () => {
    ...
    expect(STOCK_CATS['stock-a'].nameKey).toBe('category.stockA');
    expect(STOCK_CATS['stock-hk'].nameKey).toBe('category.stockHk');
    expect(STOCK_CATS['stock-us'].nameKey).toBe('category.stockUs');
  });
});
