...
import { classifySector, SECTOR_NAMES, SECTOR_PALETTE_SLOT } from "../sector";
...
describe("SECTOR_PALETTE_SLOT", () => {
  it("has entries for all common sector keys", () => {
    ...
    for (const key of keys) {
      expect(SECTOR_PALETTE_SLOT[key]).toBeDefined();
      expect(typeof SECTOR_PALETTE_SLOT[key]).toBe("number");
    }
  });

  it("telecom and communication share the same palette slot", () => {
    expect(SECTOR_PALETTE_SLOT.telecom).toBe(SECTOR_PALETTE_SLOT.communication);
  });
});

describe("SECTOR_NAMES", () => {
  it("has zh labels for all sector keys in SECTOR_PALETTE_SLOT", () => {
    for (const key of Object.keys(SECTOR_PALETTE_SLOT)) {
